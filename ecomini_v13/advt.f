      SUBROUTINE ADVT(FB,F,FMEAN,DTI2,FF,FDIF2,TSDIS,TSBDRY)
C
C*************************************************************************
C     ECOMSED MODEL
C     VERSION 1.3
...
C
      ...
      DIMENSION FDIF2(DBCM,KBM1),TSDIS(QBCM),TSBDRY(EBCM,KBM1)
...
C-------- ADD NET ADVECTION FLUXES & THEN STEP FORWARD IN TIME --------
  ...
  141 FF(I,J,K)=(FB(I,J,K)*(H(I,J)+ETB(I,J))*ART(I,J)-DTI2*FF(I,J,K))
...
C----- ADD NET ADVECTIVE FLUXES & STEP FORWARD IN TIME 
  ...
  140 FF(I,J,K)=(FBSTART(I,J,K)*(H(I,J)+ETA(I,J))*ART(I,J)-
...
C-----------------------------------------------------------------------
      ...
      JD=JDD(N)
      DO 150 K=1,KBM1
      FF(ID,JD,K)=FF(ID,JD,K)+DTI2*FDIF2(N,K)*QDIFF(N)*RAMP*
