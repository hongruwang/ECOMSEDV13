      SUBROUTINE ADVCHEM(FB,F,DTI2,FF,TSDIS,TSBDRY,FDIF,DKRATE)
C
C*************************************************************************
C     ECOMSED MODEL
C     VERSION 1.3
...
C*************************************************************************
C  INCLUDES DECAY RATE FOR NON-CONSERVATIVE PARTICLE-BOUND TRACER
C
      INCLUDE 'comdeck'
      SAVE
C
...
C
      CALL ANTIDIF_CHEM(XMFLUX,YMFLUX,ZZFLUX,FB,FF,DTI2,TSDIS,TSBDRY)
C
      DO 146 J=1,JM
      DO 146 I=1,IM
      ETA(I,J)=ETF(I,J)
...
C-----------------------------------------------------------------------
     ...
     .   VDDISTCH(N,K)/100./DZ(K)/((H(ID,JD)+ETF(ID,JD))*ART(ID,JD))
        ELSE
 ...
 6112 FORMAT(/' HYDTYPE IS  = ',A8,' THIS IS NOT A VALID OPTION '//
     *    ' FOR A NEGATIVE CHEMICAL LOAD THROUGH DIFFUSER'//
